...
public class DailySummary : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (btnNextDay != null)
            btnNextDay.clicked += OnNextDayClicked;
    ...
    private void BuildLoanUI()
    {
        ...
        debtLabel.style.color = new Color(1f, 0.65f, 0.2f);
        debtLabel.style.fontSize = 13;
        lblDebtActive = new Label("$0");
        lblDebtActive.style.color = new Color(1f, 0.65f, 0.2f);
        lblDebtActive.style.fontSize = 13;
        lblDebtActive.style.unityFontStyleAndWeight = FontStyle.Bold;
        debtSection.Add(debtLabel);
        debtSection.Add(lblDebtActive);

        loanSection = new VisualElement();
        loanSection.style.backgroundColor = new Color(0.55f, 0.08f, 0.08f, 0.6f);
        loanSection.style.borderTopLeftRadius = 8;
        loanSection.style.borderTopRightRadius = 8;
        loanSection.style.borderBottomLeftRadius = 8;
        loanSection.style.borderBottomRightRadius = 8;
        loanSection.style.paddingTop = 14;
        loanSection.style.paddingBottom = 14;
        loanSection.style.paddingLeft = 16;
        loanSection.style.paddingRight = 16;
        loanSection.style.marginBottom = 14;
        loanSection.style.display = DisplayStyle.None;

        var loanTitle = new Label("! NEGATIVE BALANCE");
        loanTitle.style.color = new Color(1f, 0.35f, 0.35f);
        loanTitle.style.fontSize = 12;
        loanTitle.style.unityFontStyleAndWeight = FontStyle.Bold;
        loanTitle.style.marginBottom = 6;

        lblLoanAmount = new Label();
        lblLoanAmount.style.color = Color.white;
        lblLoanAmount.style.fontSize = 13;
        lblLoanAmount.style.marginBottom = 4;

        var loanInterestNote = new Label();
        ...
        loanInterestNote.text = $"Daily interest: {rate:F0}% on total debt";
        loanInterestNote.style.color = new Color(0.75f, 0.75f, 0.75f);
        loanInterestNote.style.fontSize = 11;
        ...
        btnAcceptLoan.style.color = Color.white;
        btnAcceptLoan.style.fontSize = 12;
        btnAcceptLoan.style.unityFontStyleAndWeight = FontStyle.Bold;
        btnAcceptLoan.style.borderTopLeftRadius = 5;
        btnAcceptLoan.style.borderTopRightRadius = 5;
        btnAcceptLoan.style.borderBottomLeftRadius = 5;
        btnAcceptLoan.style.borderBottomRightRadius = 5;
        btnAcceptLoan.style.borderTopWidth = 0;
        btnAcceptLoan.style.borderBottomWidth = 0;
        btnAcceptLoan.style.borderLeftWidth = 0;
        btnAcceptLoan.style.borderRightWidth = 0;
        btnAcceptLoan.clicked += OnAcceptLoanClicked;

        loanSection.Add(loanTitle);
        loanSection.Add(lblLoanAmount);
        loanSection.Add(loanInterestNote);
        loanSection.Add(btnAcceptLoan);
    ...
    {
        EconomyManager.OnDailySummaryReady += ShowSummary;
    }
    ...
    private void ShowSummary(DailySummaryData data)
    {
        ...
        if (lblNet != null)
        {
            ...
            lblNet.style.color = data.net >= 0f
                ? new Color(0.47f, 0.86f, 0.47f)
    ...
    private void RefreshLoanUI()
    {
        ...
        float debt = EconomyManager.Instance.LoanDebt;
        if (debtSection != null)
        {
            bool hasDebt = debt > 0f;
            debtSection.style.display = hasDebt ? DisplayStyle.Flex : DisplayStyle.None;
            if (lblDebtActive != null) lblDebtActive.text = $"${Mathf.RoundToInt(debt)}";
        }

        float capital = EconomyManager.Instance.Capital;
