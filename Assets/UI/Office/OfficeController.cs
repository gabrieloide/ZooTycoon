...
public class OfficeController : MonoBehaviour
{
    ...
    {
        EconomyManager.OnCapitalChanged += OnCapitalChanged;
    ...
    {
        if (lblCapital != null)
            lblCapital.text = $"Available Capital: ${Mathf.RoundToInt(capital)}";
    ...
    private void RefreshContracts()
    {
        ...
        if (lblCapital != null && EconomyManager.Instance != null)
            lblCapital.text = $"Available Capital: ${Mathf.RoundToInt(EconomyManager.Instance.Capital)}";
        ...
        foreach (var license in LicenseManager.Instance.GetAllLicenses())
        {
            ...
            contractsList.Add(BuildContractCard(license));
        }
    }
    ...
    private VisualElement BuildContractCard(LicenseData license)
    {
        ...
        bool canAfford = license.cost <= 0 ||
                         (EconomyManager.Instance != null && EconomyManager.Instance.CanAfford(license.cost));

        var card = new VisualElement();
        card.style.backgroundColor = purchased
            ? new Color(0.07f, 0.22f, 0.07f)
        ...
        card.style.borderTopColor = purchased
            ? new Color(0.2f, 0.6f, 0.2f, 0.4f)
            : new Color(1f, 0.78f, 0.3f, 0.15f);
        ...
        lblName.style.marginBottom = 4;

        var lblDesc = new Label(license.description);
        lblDesc.style.color = new Color(0.65f, 0.65f, 0.65f);
        lblDesc.style.fontSize = 11;
        lblDesc.style.whiteSpace = WhiteSpace.Normal;
        lblDesc.style.marginBottom = 4;

        if (license.unlockedBiomes != null && license.unlockedBiomes.Count > 0)
        {
            var biomeNames = string.Join(", ", license.unlockedBiomes.ConvertAll(b => b != null ? b.displayName : "?"));
            var lblBiomes = new Label($"Unlocks: {biomeNames}");
            lblBiomes.style.color = new Color(1f, 0.78f, 0.3f);
            lblBiomes.style.fontSize = 10;
            info.Add(lblName);
            info.Add(lblDesc);
            info.Add(lblBiomes);
        }
        ...
        if (purchased)
        {
            ...
            badge.style.color = new Color(0.4f, 0.9f, 0.4f);
        ...
        else
        {
            ...
            btnSign.style.width = 80;
            btnSign.style.height = 52;
            btnSign.style.backgroundColor = canAfford
                ? new Color(0.85f, 0.65f, 0.1f)
                : new Color(0.25f, 0.25f, 0.25f);
            btnSign.style.color = canAfford ? new Color(0.05f, 0.05f, 0.05f) : new Color(0.5f, 0.5f, 0.5f);
            btnSign.style.fontSize = 11;
            btnSign.style.unityFontStyleAndWeight = FontStyle.Bold;
            btnSign.style.borderTopLeftRadius = 6;
            btnSign.style.borderTopRightRadius = 6;
            btnSign.style.borderBottomLeftRadius = 6;
            btnSign.style.borderBottomRightRadius = 6;
            btnSign.style.borderTopWidth = 0;
            btnSign.style.borderBottomWidth = 0;
            btnSign.style.borderLeftWidth = 0;
            btnSign.style.borderRightWidth = 0;
            btnSign.SetEnabled(canAfford);

            var captured = license;
            btnSign.clicked += () =>
            {
                LicenseManager.Instance.TryPurchase(captured);
                RefreshContracts();
            };
