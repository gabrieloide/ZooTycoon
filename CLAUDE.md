...
- **Estructura del día:**
  ...
  - **Dormir (trigger manual)** — El jugador decide cuándo terminar el día. Si duerme antes de las 22:00 → día siguiente empieza a las 6:00 con animales en estrés 0.
  - **22:00 — Auto-sleep (penalización):** Si el jugador no durmió, el juego lo manda a dormir forzado. Penalización doble: (1) el día siguiente empieza directamente a las 8:00, sin ventana de planificación; (2) todos los animales arrancan con estrés acumulado. *Futuro: indicador visual de que "vas tarde" antes de las 22:00.*
...
## 7. Estado Actual (28 jun 2026)
- ✅ Grid System O(1) · Habitat Builder con validación holográfica · Cámaras Cinemachine 3 · Avatar con estamina (sprint/drain/regen/tired) · TimeManager ciclo completo (6:00/8:00/18:00/22:00) con **VoidEventChannelSO** (SO Architecture — static events reemplazados) · **EconomyManager** · **VisitorManager** · **HUD de Finanzas** (lbl-money + lbl-visitors) · Cobro por tile al construir (HabitatBuilder.TryBuild → EconomyManager.Spend) · Shop 4 tabs · ShopDetector · **BedTrigger + SleepManager** (E-key voluntario + auto-sleep 22:00 con penalización) · **Stress AI completa**: bioma incorrecto (wrongBiomeStressRate) + familia (CompatibilityMatrix) + sobrepoblación (overcrowdingPenaltyPerTile) + vecinos hábitats (BiomeConflict) · **AnimalStressBar** (verde→rojo) · **AnimalWandering** · **AnimalPlacer** (spawning tool) · **Escape físico** al 100% estrés (Animal.TriggerEscape) + **Recaptura** (Animal.Recapture) · **DisasterManager** · InteractPrompt (E-key) · BuildCostPreview UI · **Tensión cruzada entre hábitats** (BiomeConflict en CompatibilityMatrix SO + cachedNeighborTension por evento de build) · **Distancia mínima entre hábitats** (preview rojo + bloqueo en TryBuild, configurable en SO).
- ⏳ Pendiente: visual placeholder valla rota · penalización económica por desastre (visitantes huyen) · firma de licencias (lógica) · La Oficina UI · Pantalla de liquidación diaria · Préstamo de emergencia · Main Menu · Tutorial · Sonidos.
- 🔧 Inspector pendiente (Gabriel): asignar `CompatibilityMatrix` SO a `BuildController.globalMatrix` · configurar entradas `BiomeConflict` en el SO · asignar `availableLicenses` en GameHUDController · asignar `VisitorConfig` y `EconomyConfig` en VisitorManager.
- 📅 Demo en Itch.io: 16 jul 2026. Checkpoint de diversión: fin de semana 3 (2 jul — esta semana).